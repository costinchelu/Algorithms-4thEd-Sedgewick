 ...
 *  {@link UnionFindUF}, {@link QuickFindUF}, and {@link QuickUnionUF}.
...
public class WeightedQuickUnionUF {
    ...
    private final int[] subtree;    // size[i] = number of elements in subtree rooted at i
    ...
    public void union(int p, int q) {
        ...
        if (subtree[rootP] < subtree[rootQ]) {
            arr[rootP] = rootQ;
            subtree[rootQ] += subtree[rootP];
        }
        else {
            arr[rootQ] = rootP;
            subtree[rootP] += subtree[rootQ];
        }
        count--;
    }
